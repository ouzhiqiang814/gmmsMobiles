	...
	MobileCalendar.prototype = {
		init: function(params) {
			...
			this.trigger = document.querySelector(params.trigger);
					//限制时间
			var creatData = new Date();
			var minyear = creatData.getFullYear()-15;
			var maxyear = creatData.getFullYear();
			var minData = minyear+'-01-1';
			var maxData = maxyear+'-12-31';
			document.getElementById("demo1").setAttribute("data-lcalendar",minData+","+maxData);
		...
		bindEvent: function(type) {
			...
			function popupDate(e) {
				...
				_self.gearDate.innerHTML = '<div class="date_ctrl slideInUp">' +
					...
					'</div>' +
					'<div>' +
					'<div class="gear date_dd" data-datetype="date_dd" style="display:none"></div>' +					
					'</div>' +
					'</div>' +
					'</div>' +
					'</div>' +
			...
			function finishMobileDate(e) {
				...
				date_mm = date_mm > 9 ? date_mm : date_mm;

				_self.trigger.value = (date_yy % passY + _self.minY) + "-" + date_mm ;
				var year = date_yy % passY + _self.minY;
				var month = date_mm;
